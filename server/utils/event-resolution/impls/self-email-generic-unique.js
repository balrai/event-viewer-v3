...
export class SelfEmailGenericUniqueResolution extends IPathResolutionHandler {
    ...
    switch (path) {
      ...
      case "main":
        if (!userProfile) {
          ...
          return onShowFlow(endShow, preShow, main, rehearsal, showStatus);
        }
      case "registration":
        if (!userProfile) {
          return new PathResolutionResult(
  ...
  canHandle(authPreset) {
    switch (authPreset) {
      case AuthPreset.SELF_EMAIL_UNIQUE:
      case AuthPreset.SELF_EMAIL_GENERIC:
        return true;
    }
    return false;
  }
...
function onShowFlow(endShow, preShow, main, rehearsal, showStatus) {
  if (shouldShowEndShow(endShow)) {
    return new PathResolutionResult(
      true,
      {
